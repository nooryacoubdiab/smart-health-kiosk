...
function validateVitals({ bp_sys, bp_dia, spo2, temp, pulse }) {
  if (bp_sys == null || bp_dia == null || temp == null) {
    return 'Blood pressure and temperature are required.';
  }
...
// ---------- GET /api/latest-vitals/:national_id (fixed) ----------
// Only returns a reading recorded AFTER the current session started for this
// exact patient — a stale reading from an earlier test can no longer "leak" in.
app.get('/api/latest-vitals/:national_id', (req, res) => {
  ...
  `).get(patient.id, sinceTime);

  res.json({ reading: reading || null });
});

// ---------- POST /api/gov-ticket ----------
// Non-medical service ticket (e.g. Health Clearance Certificate) — always
// routes to General, Standard queue, no vitals or triage involved.
app.post('/api/gov-ticket', (req, res) => {
  const { national_id } = req.body;
  const patient = db.prepare('SELECT * FROM patients WHERE national_id = ?').get(national_id);
  if (!patient) return res.status(404).json({ error: 'Patient not found' });

  const ticketNo = generateTicketNo('standard', 'general');
  const result = db.prepare(
    `INSERT INTO queues (patient_id, department, queue_type, ticket_no) VALUES (?, 'general', 'standard', ?)`
  ).run(patient.id, ticketNo);

  const priorityAhead = db.prepare(`
    SELECT COUNT(*) AS c FROM queues WHERE department = 'general' AND queue_type = 'priority' AND status = 'waiting'
  `).get().c;
  const standardAhead = db.prepare(`
    SELECT COUNT(*) AS c FROM queues WHERE department = 'general' AND queue_type = 'standard' AND status = 'waiting' AND id != ?
  `).get(result.lastInsertRowid).c;
  const estimatedWaitMinutes = (priorityAhead + standardAhead) * WAIT_MINUTES_PER_PATIENT;
